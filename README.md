...
## P2P
In the original decentralised spirit of the Memex, it is possible to store these JSON-LD annotations in a peer-to-peer content-addressable network such as [IPFS](https://ipfs.io/). To do so, the `@id` property must be excluded as per [this discussion](https://github.com/ipfs/notes/issues/152#issuecomment-239153915).
Content IRIs can take the form `fs:/ipfs/<hash>/`. The consuming application is responsible for interpreting and routing these IRIs.
